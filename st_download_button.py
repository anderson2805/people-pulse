...
def download_button(object_to_download, download_filename, button_text, pickle_it=False):
    ...
    if pickle_it:
        ...
        elif isinstance(object_to_download, Workbook):
            buffer = io.BytesIO()
            object_to_download.save(buffer)
            object_to_download = buffer.getvalue()

        elif isinstance(object_to_download, (str, int, float, bool)):
...
if __name__ == '__main__':
    ...
    if st.checkbox('Download object from memory'):
        st.write('~> Use if you want to save some data from memory (e.g. pd.DataFrame, dict, list, str, int, openpyxl.Workbook)')
        ...
        filename = st.text_input('Enter output filename and ext (e.g. my-dataframe.csv, my-file.json, my-list.txt, my-workbook.xlsx)', 'my-file.json')
